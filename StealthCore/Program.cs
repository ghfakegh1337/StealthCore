...
class Program
{
    ...
    static void SelectGame(ref char gameChoice)
    {
        while (true)
        {
            ...
            {
                Console.WriteLine("Mistake! Select 1 (Rusticaland) or 2 (GTA5). Try again.");
            }
        }
    }
    ...
    static async Task Rust(string directoryPath)
    {
        ...
        Console.WriteLine("Type \"inject\" to load cheat into the process:");
        ...
        do
        {
            ...
            if (command != null && command.StartsWith("inject", StringComparison.OrdinalIgnoreCase))
            {
                InjectDLL("RustClient", directoryPath + '\\' + SYRustRandomFileName);
            }
            ...
            else if (command != null && command.StartsWith("clear", StringComparison.OrdinalIgnoreCase))
            {
                // Чистка папок
            ...
            {
                Console.WriteLine("You entered an invalid command. Please type 'inject/folder/clear/exit'.");
            }

        } while (command == null || !command.StartsWith("clear", StringComparison.OrdinalIgnoreCase));
    }

    static async Task NFgta5(string directoryPath)
    {
        // Создаем важную папку
        Directory.CreateDirectory(directoryPath);

        // Определяем файлы с оригинальными именами
        string NFLoaderOriginalFileName = "hraAnrq.exe";

        // Скачиваем файлы с оригинальными именами
        Console.WriteLine("Downloading NF Loader...");
        await DownloadFileAsync("https://github.com/ghfakegh1337/StealthCore/raw/refs/heads/main/files/gta5/NF/hraAnrq.exe", Path.Combine(directoryPath, NFLoaderOriginalFileName));
        Console.WriteLine("NF Loader downloaded.");

        // Переименовываем скачанные файлы в случайные имена
        string NFLoaderSavedFileName = GenerateRandomFileName(10);
        string NFLoaderRandomFileName = NFLoaderSavedFileName + ".exe";

        File.Move(Path.Combine(directoryPath, NFLoaderOriginalFileName), Path.Combine(directoryPath, NFLoaderRandomFileName));

        Console.WriteLine($"Renamed {NFLoaderOriginalFileName} to {NFLoaderRandomFileName}.");

        // Проверяем ввод пользователя для очистки
        Console.WriteLine("Type 'open' to open the cheat:");
        Console.WriteLine("Type 'folder' to open the cheat folder:");
        Console.WriteLine("Type 'clear' to clean:");
        string? command;
        ...
        {
            command = Console.ReadLine();
            if (command != null && command.StartsWith("open", StringComparison.OrdinalIgnoreCase))
            {
                string fullPath = Path.Combine(directoryPath, NFLoaderRandomFileName);
            ...
            else if (command != null && command.StartsWith("clear", StringComparison.OrdinalIgnoreCase))
            {
                ...
                KillProcessByName(NFLoaderSavedFileName);
                
                // Чистка папок
                Console.WriteLine("Cleaning folders...");
                ForceClearDirectory(@"C:\scripts");
                ForceClearDirectory(directoryPath);
                ClearNvidiaControlPanel();
    ...
    static bool InjectDLL(string processName, string dllPath)
    {
        ...
        {
            Console.WriteLine($"Process {processName}.exe is not running!");
            return false;
        }
        ...
        IntPtr allocMem = VirtualAllocEx(hProcess, IntPtr.Zero, (uint)(dllPath.Length + 1), 0x1000 | 0x2000, 0x40);
        ...
        Console.WriteLine($"DLL ({dllPath}) successfully injected into {processName}.exe");
        return true;
    }
        ...
        try
        {
            if (Directory.Exists(tempPath))
            {
                ...
                foreach (string dir in Directory.GetDirectories(tempPath))
                {
                    try
                    {
                        Directory.Delete(dir, true); // true для рекурсивного удаления
            ...
            {
                Console.WriteLine("Temp folder does not exist.");
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Error accessing temp folder: {ex.Message}");
        }
    }
